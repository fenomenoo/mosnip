...
export function transcribeAudio(audioPath: string, tempDir: string): { transcriptPath: string; srtPath: string } {
  ...
  log.success(`Transcript saved (~${wordCount} words)`);
  if (fs.existsSync(srtPath)) log.info('SRT subtitle file generated');

  return { transcriptPath, srtPath: fs.existsSync(srtPath) ? srtPath : '' };
}

export function runTranscription(videoPath: string, tempDir: string): { transcriptPath: string; srtPath: string } {
  const audioPath = extractAudio(videoPath, tempDir);
  const result = transcribeAudio(audioPath, tempDir);
